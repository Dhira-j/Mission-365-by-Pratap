...
function validateForm(event) {
    ...
    if (username.value.trim().length < 3) {
        ...
        popupModel("Failed","Name must be at least 3 characters","error");
    ...
    if (!phone.value.match(phonePattern)) {
        ...
        popupModel("Failed","Phone must be exactly 10 digits","error");
    ...
    var yyyy = today.getFullYear();
    var mm = today.toLocaleString('en-IN', { month: 'long' });
    var dd = String(today.getDate()).padStart(2, '0');
    ...
    if (dateTaken) {
        ...
        event.preventDefault();
        const html= `Sorry, this date is already fully booked by another client.`;
        
        popupModel("Date Unavailable 📅",html,"error");
        isValid = false;
    } 
    else if (userHasBooking) {
        // ERROR: This specific user is trying to book a second time
        event.preventDefault();
        const html=`Hi <b>${username.value}</b>, you already have a booking on <b>${userHasBooking.date}</b>.`
        popupModel("Multiple Bookings Not Allowed ⚠️",html,"error");
        ...
        const html= `<b>${username.value}</b>, your appointment is set for ${date.value}.`;
        popupModel("Booking Confirmed 🎉",html,"success");
...
function popupModel(...value){
    var[title,message,status]=value;
    const modal=document.querySelector(".modal-overlay");
    const icon=document.querySelector(".icons");
    const glassEffect=document.querySelector(".glassEffect");
    const modalTitle=document.querySelector(".modal-title");
    const modalMsg=document.querySelector(".modal-message");
    modal.style.display="flex";
    icon.className = "icons bi";
    if(status==="success"){
       glassEffect.style.borderColor="#d4a373";
       ...
       icon.style.color="#d4a373";
        ...
        glassEffect.style.borderColor="#E35336";
       modalTitle.innerText=title;
       modalMsg.innerHTML=message;
       icon.classList.add("bi-exclamation-triangle-fill");
       icon.style.color="#FF8559";
    }
